...
def conv_network():
	...
	print(
		"If you see any 'can't read file' error messages below, please ignore them for now, this is normal behaviour in this early build")
	print("------------------------------------------------")
	...
	for i in range(len(training_data)):
		...
		for j in range(len(training_data[i])):
			...
			if print_counter % 1000 == 0:
				printable_output = sess.run(output,
											feed_dict={training_input: batch_input, training_output: batch_output,
													   keep_prob: 1})
